...
* Deploy Nexus3 repository and Nexus docker registry, SonarQube.

* Create a custom Jenkins slave image with skopeo.

* Clone this git Repository

* Run either of the OpenShift template for creating BuildConfig with Jenkins pipeline.

  ```
  cd openshift-tasks/
  oc process -f openshift/jenkins-pipeline-embedded.yml -p GUID=$GUID | oc create -f -

  Or

  oc process -f openshift/jenkins-pipeline-jenkisfilepath.yml -p GUID=$GUID | oc create -f -
  ```
